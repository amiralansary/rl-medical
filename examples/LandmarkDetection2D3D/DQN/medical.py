...
class MedicalPlayer(RLEnvironment):
    ...
    def __init__(self, directory=None, viz=False, train=False,
                 screen_dims=(37,37), nullop_start=30,
                 location_history_length=16, save_gif=False):
        """
    ...
    def new_random_game(self):
        ...
        # logger.info("filepath {} - target_loc {} ".format(self.filepath, self._target_loc))

        # image volume size
        self._image_dims = self._game_img.dims
        self._loc_dims = np.array((self.screen_dims[0]+1, self.screen_dims[1]+1, 1, self._image_dims[0]-self.screen_dims[0]-1, self._image_dims[1]-self.screen_dims[1]-1, self._image_dims[2]-1))
    ...
    def action(self, act):
        ...
        else: # check if agent oscillates
    ...
    def get_screen(self):
        ...
        screen = np.zeros((self.screen_dims[0],self.screen_dims[1]))
        ...
        if z>=self._image_dims[2]:
            z = self._image_dims[2] - 1
        ...
        screen[screen_xmin:screen_xmax, screen_ymin:screen_ymax] = self._game_img.data[xmin:xmax, ymin:ymax, z]
