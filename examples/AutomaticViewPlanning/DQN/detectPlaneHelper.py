...
def getACPCPlaneFromLandmarks(sitk_image3d, origin,
                              ac_point,pc_point,midsag_point,
                              plane_size, spacing):
...
def recalcDirectionalAngles(params):
    a, b, c = [np.cos(np.deg2rad(params[0])),
               np.cos(np.deg2rad(params[1])),
               np.cos(np.deg2rad(params[2]))]
    # find plane norm vector
    plane_norm = normalizeUnitVector(np.array((a,b,c)))
    params = [np.rad2deg(np.arccos(plane_norm[0])),
              np.rad2deg(np.arccos(plane_norm[1])),
              np.rad2deg(np.arccos(plane_norm[2]))]
    return params
...
def calcScaledDistTwoParams(params1, params2, scale_angle, scale_dist):
    ...
    dist = params1 - params2
    if (scale_angle==0):  scale_angle = np.finfo('float').eps
    if (scale_dist==0):  scale_dist = np.finfo('float').eps
    dist[:-1] /= scale_angle
    dist[-1] /= scale_dist
